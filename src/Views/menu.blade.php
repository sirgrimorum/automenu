...
<nav class="navbar navbar-expand-lg {{\Illuminate\Support\Arr::get($config,"classes.navbar_extra")}}" role="navigation">
    ...
    @endif
    <button class="navbar-toggler {{\Illuminate\Support\Arr::get($config,"classes.button_izquierdo")}}" type="button" data-toggle="collapse" data-target="#{{$config['id']}}" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
        <i class="{{\Illuminate\Support\Arr::get($config,"icons.left",'fa fa-bars')}}"></i>
    </button>
    <div class="collapse navbar-collapse {{\Illuminate\Support\Arr::get($config,"classes.navbar_collapse")}}" id="{{$config['id']}}">
        @if(!empty($config["menu"]["izquierdo"]))
    ...
    <div class="dropdown">
        @if(!empty($config["menu"]["izquierdo"]))
        <button class="nav-item btn {{\Illuminate\Support\Arr::get($config,"classes.button_izquierdo")}}" type="button" id="{{$config['id']}}_i_l" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
            <i class="{{\Illuminate\Support\Arr::get($config,"icons.left",'fa fa-bars')}}"></i>
        </button>
        <div class="dropdown-menu {{\Illuminate\Support\Arr::get($config,"classes.navbar_collapse")}}" id="{{$config['id']}}_i" aria-labelledby="{{$config['id']}}_i_l">
    ...
    <div class="dropdown">
        @if(!empty($config["menu"]["derecho"]))
        <button class="nav-item btn {{\Illuminate\Support\Arr::get($config,"classes.button_derecho")}}" type="button" id="{{$config['id']}}_d_l" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
            <i class="{{\Illuminate\Support\Arr::get($config,"icons.right",'fa fa-bars')}}"></i>
        </button>
        <div class="dropdown-menu {{\Illuminate\Support\Arr::get($config,"classes.navbar_collapse")}}" style="right: 0px;left: auto;" id="{{$config['id']}}_d" aria-labelledby="{{$config['id']}}_d_l">
